...
timer_interrupt(struct intr_frame *args)
{
    ...
    elem_iter = list_begin(&wait_list);
    while (elem_iter != list_end(&wait_list))
    {
        thread_iter = list_entry(elem_iter, struct thread, elem);
        if (thread_iter->expired_time > ticks)
        {
            elem_iter = list_next(elem_iter);
        }
        else
        {
            elem_iter = list_remove(elem_iter);
    ...
    if (thread_aging || thread_mlfqs)
    {
        ...
        if (timer_ticks() % 4 == 0)
        {
            ...
            {
                thread_set_load_avg_and_recent_cpu();
            }
            set_priority();
        }
    }
